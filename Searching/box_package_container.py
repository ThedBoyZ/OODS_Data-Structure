...
def minWeigthbox(weigth: list, number: int) -> int :
    if number == 1:
        return sum(weigth)
    
    minWeigth_return = 1000000
    for i in range(len(weigth)):
        if len(weigth[i:]) < number-1:
            break
        curNumber  = sum(weigth[:i])
        nextNumber = minWeigthbox(weigth[i:], number-1)
        minWeigth_return = min(max(curNumber, nextNumber),minWeigth_return)
    return minWeigth_return

def main():
    inp = input("Enter Input : ").split('/')
    we, nu = list(map(int, inp[0].split())), int(inp[1])
    print(f"Minimum weigth for {nu} box(es) = {minWeigthbox(we,nu)}")
